...
const CATEGORIES = {
  ...
  CONTRACTS_PRICING: 'العقود والأسعار',
  BENEFITS: 'المنافع',
...
export default function UserCreate() {
  ...
  useEffect(() => {
    let mounted = true;
    Promise.all([
      accessControlService.getPermissionCatalogue(),
      accessControlService.getRoleTemplates(),
      ...
      .catch(
        (error) =>
          mounted && setLoadError(error?.response?.data?.message || 'تعذر تحميل عقد الصلاحيات؛ تم إيقاف الإنشاء حمايةً من إعداد ناقص.')
      )
  ...
  const submit = async () => {
    ...
    const permissionOverrides = (editing ? changedOverrides : Object.keys(overrides)).map((permissionCode) => ({
      permissionCode,
      mode: overrides[permissionCode] || 'INHERIT',
      reason: changeReason.trim()
    }));
    const user = {
      username: form.username.trim(),
      ...(editing ? { active: form.active } : { password: form.password }),
      fullName: form.fullName.trim(),
      email: form.email.trim(),
      phone: form.phone.trim() || null,
      userType: form.userType,
      employerId: needsEmployer(form.userType) ? Number(form.employerId) : null,
      providerId: needsProvider(form.userType) ? Number(form.providerId) : null
    };
    try {
      setSaving(true);
      ...
      openSnackbar({
        open: true,
        message: editing ? 'تم تحديث المستخدم والنطاق والصلاحيات ذرياً' : 'تم إنشاء المستخدم وتطبيق النطاق والصلاحيات',
        variant: 'alert',
        alert: { color: 'success' }
      });
      ...
      openSnackbar({
        open: true,
        message: error?.response?.data?.messageAr || error?.response?.data?.message || 'تعذر إنشاء المستخدم',
        variant: 'alert',
        alert: { color: 'error' }
      });
    } finally {
  ...
  return (
    <Box dir="rtl">
      <ModernPageHeader
        ...
        subtitle="الدور والنطاق والصلاحيات في قرار واحد قابل للمراجعة"
        breadcrumbs={[
          { label: 'الرئيسية', path: '/' },
          { label: 'المستخدمون', path: '/admin/users' },
          { label: editing ? 'تعديل' : 'إضافة' }
        ]}
        actions={
      ...
      <MainCard contentSX={{ p: { xs: 2, md: 3 } }}>
        ...
        {step === 0 && (
          <Stack spacing={3}>
            <Box>
              <Typography variant="h3">الدور الأساسي</Typography>
              <Typography color="text.secondary">قالب بداية، وليس حكماً نهائياً على الصلاحيات.</Typography>
            </Box>
            <Grid container spacing={1.5}>
              {roles.map((role) => (
                <Grid item xs={12} sm={6} md={4} key={role.roleCode}>
                  <Card
                    variant="outlined"
                    sx={{
                      height: '100%',
                      borderColor: form.userType === role.roleCode ? 'primary.main' : 'divider',
                      bgcolor: form.userType === role.roleCode ? 'primary.lighter' : 'background.paper'
                    }}
                  >
                    <CardActionArea sx={{ height: '100%' }} onClick={() => change('userType', role.roleCode)}>
                      <CardContent>
                        <Stack direction="row" justifyContent="space-between">
                          <AdminPanelSettingsOutlinedIcon color="primary" />
                          <Chip size="small" label={`${role.permissionCodes.length} صلاحية`} />
                        </Stack>
                        <Typography variant="h5" mt={1}>
                          {RoleDisplayNames[role.roleCode]?.ar || role.displayNameAr}
                        </Typography>
                        <Typography variant="body2" color="text.secondary" mt={1}>
                          {ROLE_HELP[role.roleCode]}
                        </Typography>
                      </CardContent>
                    </CardActionArea>
                  </Card>
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} md={6}>
                <TextField fullWidth label="الهاتف" value={form.phone} onChange={(e) => change('phone', e.target.value)} />
              </Grid>
              {!editing && (
                <>
                  <Grid item xs={12} md={6}>
        ...
        {step === 1 && (
          <Stack spacing={3} sx={{ maxWidth: 850, mx: 'auto' }}>
            <Box textAlign="center">
              <Typography variant="h3">نطاق الوصول</Typography>
              <Typography color="text.secondary">النطاق يحدد أين يعمل المستخدم، ولا يمنحه عمليات إضافية.</Typography>
            </Box>
            {needsEmployer(form.userType) && (
              <EmployerSelectField
                value={form.employerId}
                onChange={(value) => change('employerId', value?.target ? value.target.value : value)}
                error={!!errors.employerId}
                helperText={errors.employerId}
                required
              />
            )}
            {needsProvider(form.userType) && (
              <ProviderSelectField
                value={form.providerId}
                onChange={(value) => change('providerId', value)}
                options={providers}
            ...
            {!needsEmployer(form.userType) && !needsProvider(form.userType) && (
              <Alert severity="info">دور داخلي عام؛ تبقى كل عملية حساسة مقيدة بصلاحيتها وقواعد قسمها.</Alert>
            )}
          </Stack>
        )}
        ...
        {step === 2 && (
          <Stack spacing={2}>
            <Box>
              <Typography variant="h3">الصلاحيات الفعلية</Typography>
              <Typography color="text.secondary">السحب الصريح يتقدم على الدور. استخدم الاستثناء فقط لحاجة موثقة.</Typography>
            </Box>
              ...
              <Table stickyHeader size="small">
                ...
                <TableBody>
                  {visiblePermissions.map((permission) => {
                    ...
                    const enabled = effective.has(permission.code);
                    return (
                      <TableRow key={permission.code}>
                        <TableCell>{CATEGORIES[permission.category]}</TableCell>
                        <TableCell>
                          <Typography fontWeight={600}>
                            {permission.displayNameAr}{' '}
                            {permission.sensitive && (
                              <WarningAmberRoundedIcon color="warning" sx={{ fontSize: 16, verticalAlign: 'middle' }} />
                            )}
                          </Typography>
                        ...
                        <TableCell align="center">
                          <ToggleButtonGroup
                            ...
                            size="small"
                            value={mode}
                            onChange={(_, value) => value && setOverrides((old) => ({ ...old, [permission.code]: value }))}
                          >
                            <ToggleButton value="INHERIT">من الدور</ToggleButton>
                            <ToggleButton value="GRANT" color="success">
                              منح
                            </ToggleButton>
                            <ToggleButton value="REVOKE" color="error">
                              سحب
                            </ToggleButton>
                          </ToggleButtonGroup>
                        </TableCell>
            ...
            <Grid item xs={12} md={8}>
              <Stack spacing={2}>
                <Alert severity="warning" icon={<SecurityOutlinedIcon />}>
                  أي تعديل أمني يسحب جلسات المستخدم فوراً بعد نجاح المعاملة كاملة.
                </Alert>
                <Paper variant="outlined" sx={{ p: 2 }}>
                  <Typography variant="h4">ملخص الحساب</Typography>
                  <Grid container spacing={2} mt={0.5}>
                    <Grid item xs={6}>
                      الاسم: <b>{form.fullName}</b>
                    </Grid>
                    <Grid item xs={6}>
                      المستخدم: <b>{form.username}</b>
                    </Grid>
                    <Grid item xs={6}>
                      الدور: <b>{RoleDisplayNames[form.userType]?.ar}</b>
                    </Grid>
                    <Grid item xs={6}>
                      النطاق:{' '}
                      <b>
                ...
                <Paper variant="outlined" sx={{ p: 2 }}>
                  <Typography variant="h4" mb={1}>
                    الاختلاف عن قالب الدور
                  </Typography>
                  {Object.entries(overrides).some(([, mode]) => mode !== 'INHERIT') ? (
                    <Stack direction="row" gap={1} flexWrap="wrap">
                      {Object.entries(overrides)
                        ...
                        .map(([code, mode]) => (
                          <Chip
                            key={code}
                            color={mode === 'GRANT' ? 'success' : 'error'}
                            label={`${catalogue.find((p) => p.code === code)?.displayNameAr}: ${mode === 'GRANT' ? 'منح' : 'سحب'}`}
                          />
                        ))}
                    </Stack>
                  ) : (
                    <Typography color="text.secondary">لا توجد استثناءات شخصية.</Typography>
                  )}
                  <TextField
                    sx={{ mt: 2 }}
                    fullWidth
                    required={editing || Object.keys(overrides).length > 0}
                    multiline
                    minRows={2}
                    label={editing ? 'سبب تعديل الحساب' : 'سبب الاستثناءات الشخصية'}
                    value={changeReason}
                    onChange={(event) => {
                      setChangeReason(event.target.value);
                      setErrors((old) => ({ ...old, changeReason: undefined }));
                    }}
                    error={!!errors.changeReason}
                    helperText={errors.changeReason || 'سيظهر في سجل التدقيق.'}
                  />
                </Paper>
              </Stack>
            </Grid>
            <Grid item xs={12} md={4}>
              <Card sx={{ bgcolor: 'success.lighter' }}>
                <CardContent>
                  ...
                  <Typography variant="h1">{effective.size}</Typography>
                  <Typography>صلاحية فعّالة</Typography>
                </CardContent>
              </Card>
            </Grid>
          </Grid>
        ...
        <Divider sx={{ my: 3 }} />
        <Stack direction="row" justifyContent="space-between">
          <Button variant="outlined" disabled={!step || saving} onClick={() => setStep((value) => value - 1)} startIcon={<ArrowBackIcon />}>
            السابق
          </Button>
          {step < 3 ? (
            <Button variant="contained" disabled={!!loadError} onClick={next} endIcon={<ArrowForwardIcon />}>
              التالي
            </Button>
          ) : (
            <Button
