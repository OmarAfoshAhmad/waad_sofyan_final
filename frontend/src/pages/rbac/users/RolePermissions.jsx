      ...
      <Grid item xs={12} md={9}>
        <MainCard title="الصلاحيات الموروثة من الدور">
          {!selectedRole ? <Alert severity="info">اختر دورًا لعرض صلاحياته.</Alert> : <Stack spacing={2}>
            ...
            <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} alignItems="flex-start"><TextField fullWidth required label="سبب تعديل قالب الدور" value={reason} onChange={(event) => setReason(event.target.value)} helperText="يُحفظ في سجل التدقيق ولا يمكن تعديله أو حذفه." /><Chip color={changed ? 'warning' : 'success'} label={changed ? 'تغييرات غير محفوظة' : 'القالب محفوظ'} /><PermissionGuard requiredPermission="ROLE_PERMISSION_MANAGE"><Button variant="contained" startIcon={saving ? <CircularProgress size={18} color="inherit" /> : <SaveOutlinedIcon />} disabled={!changed || !reason.trim() || saving} onClick={save}>حفظ</Button></PermissionGuard></Stack>
          </Stack>}
        </MainCard>
      </Grid>
    </Grid>
