...
const RejectedBatchPrintReport = forwardRef(({ claims, employer, provider, month, year, batchCode }, ref) => {
  ...
  const rejectedClaims = (claims || []).filter((c) => c.status === 'REJECTED' || (c.refusedAmount && c.refusedAmount > 0));

  if (rejectedClaims.length === 0) {
    return (
      <div
        ref={ref}
        style={{ position: 'fixed', bottom: '-9999px', left: 0, zIndex: -1, overflow: 'hidden' }}
        className="print-content-wrapper-rejected"
      >
        <Typography variant="h6" sx={{ textAlign: 'center', p: '2.5rem' }}>
          لا توجد مطالبات مرفوضة في هذه الدفعة
        </Typography>
      </div>
    );
  }
  ...
  rejectedClaims.forEach((c) => {
    ...
    const rejectAmt = c.status === 'REJECTED' && (c.refusedAmount === null || c.refusedAmount === 0) ? req : c.refusedAmount || 0;
        ...
        <div key={pIdx} className="patient-block">
          <div className="patient-info-bar">
            <div className="pi-item">
              <strong>المستفيد:</strong> {patient.memberName}
            </div>
            <div className="pi-item">
              <strong>رقم التأمين:</strong> {patient.memberNumber}
            </div>
          </div>
