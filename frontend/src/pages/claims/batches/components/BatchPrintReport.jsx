...
const buildCompanyTitle = (name, type) => {
  ...
  const cleanType = String(type || 'لادارة النفقات الطبية')
    .replace(/\s*\(.*?\)\s*/g, '')
    .trim();
...
const extractServices = (claim) => {
  ...
  const claimRejected = toNumber(
    claim.status === 'REJECTED' && !claim.refusedAmount ? claimGross : (claim.refusedAmount ?? claim.rejectedAmount)
  );
...
const BatchPrintReport = forwardRef(({ claims, employer, provider, month, year, batchCode }, ref) => {
  ...
  return (
    <div
      ref={ref}
      style={{ position: 'fixed', bottom: '-9999px', left: 0, zIndex: -1, width: '210mm', overflow: 'hidden' }}
    ...
    >
      ...
      <div className="a4-sheet page-break">
        ...
        <div className="cover-body">{introText}</div>

        <div className="summary-section">
          <div className="summary-line">
            • إجمالي القيمة المقدمة من المرفق: <strong>{fmtAmount(totalGross)}</strong>
          </div>
          <div className="summary-line">
            • إجمالي القيمة الغير مستحقة (المرفوضة): <strong>{fmtAmount(totalRejected)}</strong>
          </div>
          <div className="summary-line">
            • إجمالي نصيب المؤمن: <strong>{fmtAmount(totalPatientShare)}</strong>
          </div>
          <div className="summary-line">
            • صافي القيمة المستحقة للمرفق: <strong>{fmtAmount(totalNet)}</strong>
          </div>
          <div className="summary-line">
            • عدد المطالبات المستلمة: <strong>[{totalClaimsCount}]</strong>
          </div>
        </div>

        <div className="cover-note">{REPORT_TEXT.note}</div>
      ...
      {detailedClaims.map((item, pIdx) => (
        <div key={pIdx} className={`a4-sheet ${pIdx < detailedClaims.length - 1 ? 'page-break' : ''}`}>
          <div className="print-header" style={{ marginBottom: '0.75rem' }}>
            ...
            <div className="print-title" style={{ fontSize: '0.875rem' }}>
              {fullTitle}
            </div>
          </div>
          ...
          <div className="patient-block" style={{ marginBottom: 0 }}>
            <div className="patient-header">
              <div className="ph-col" style={{ flex: 0.8 }}>
                <strong>No.:</strong> <br /> {item.claimRef}
              </div>
              <div className="ph-col">
                <strong>Origin No.:</strong> <br /> {item.originRef}
              </div>
            </div>
            <div className="ph-details">
              <div className="ph-details-row">
                <strong>Insurance Number:</strong> &nbsp; {item.memberNumber}
              </div>
              <div className="ph-details-row">
                <strong>Patient Name:</strong> &nbsp; {item.memberName}
              </div>
            </div>
            <div className="ph-details">
              <div className="ph-details-row">
              ...
              <tbody>
                {item.services.map((srv, sIdx) => (
                  <tr key={sIdx}>
                    ...
                    <td className="reason-cell" style={{ fontSize: '0.75rem' }}>
                      {srv.rejectionReason || ''}
                    </td>
                  </tr>
                ))}
                <tr className="subtotal-row">
                  <td colSpan={2} style={{ textAlign: 'left', paddingRight: '1.25rem' }}>
                    SUBTOTAL
                  </td>
          ...
          {pIdx === detailedClaims.length - 1 && (
            <div className="global-total">
              ...
              <div className="gt-col gross">
                Gross
                <br />
                {fmtAmount(totalGross)}
              </div>
              <div className="gt-col net">
                Net
                <br />
                {fmtAmount(totalNet)}
              </div>
              <div className="gt-col rejected">
                Rejected
                <br />
                {fmtAmount(totalRejected)}
              </div>
            </div>
          )}
        </div>
