...
import { Box, Paper, Stack, Typography, Button, Divider, CircularProgress, IconButton, Tooltip, Pagination, alpha } from '@mui/material';
import { History as HistoryIcon, Add as AddIcon, Delete as DeleteIcon, Receipt as ReceiptIcon } from '@mui/icons-material';
    ...
    <Box sx={{ width: '17.5rem', flexShrink: 0, display: 'flex', flexDirection: 'column', order: -1 }}>
      ...
      >
          ...
          <Tooltip title={!isBatchOpen ? 'الدفعة مغلقة — لا يمكن إضافة مطالبات جديدة' : ''}>
            <span>
              <Button
                ...
                variant="contained"
                color="primary"
                startIcon={<AddIcon sx={{ fontSize: '0.8125rem' }} />}
        ...
        <Typography variant="caption" color="text.disabled" sx={{ fontSize: '0.75rem', mb: 1, display: 'block' }}>
          {currentBatch
            ? `${currentBatch.monthLabel} ${currentBatch.batchYear} — ${currentBatch.claimsCount || 0} مطالبة`
            : `${monthLabel} ${year}`}
        </Typography>
        <Divider sx={{ mb: 1 }} />
