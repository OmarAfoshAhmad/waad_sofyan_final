...
export default function ClaimBatchDetail() {
  ...
  const hardDeleteMutation = useMutation({
    ...
    onError: (err) => {
      enqueueSnackbar(err?.response?.data?.messageAr || err?.response?.data?.message || 'حدث خطأ أثناء الحذف النهائي', {
        variant: 'error'
      });
    }
  });
  ...
  const handleRejectedReport = async () => {
    ...
    const detailed = await Promise.all(
      claims.map(async (c) => {
        try {
          return { ...c, ...(await claimsService.getById(c.id)) };
        } catch {
          return c;
        }
      })
    ...
    const rejectedIds = detailed
      .filter((c) => {
        if (c.lines && c.lines.length > 0) {
          return c.lines.some((l) => l.rejected === true || (l.refusedAmount != null && parseFloat(l.refusedAmount) > 0));
        }
        return (
          (c.rejectedAmount != null && parseFloat(c.rejectedAmount) > 0) || (c.totalRejected != null && parseFloat(c.totalRejected) > 0)
        );
      })
      .map((c) => c.id);
        ...
        return (
          <Tooltip title={reasonText || ''} arrow placement="top">
            <Typography
              ...
              color="error.main"
              fontWeight={400}
              sx={{ cursor: reasonText ? 'help' : 'default', textDecoration: reasonText ? 'underline dotted' : 'none' }}
            >
      ...
      case 'paid':
        ...
        return (
          <Typography variant="body2" color="secondary.main" fontWeight={600}>
            {(claim.netProviderAmount || 0).toFixed(2)}
          </Typography>
        );
      case 'actions':
        return (
          <Stack direction="row" spacing={0.5} justifyContent="center">
            <Tooltip title={claim.status === 'NEEDS_CORRECTION' ? 'فتح للتصحيح وإعادة الاعتماد' : 'عرض / تعديل'}>
              <IconButton
                ...
                onClick={() =>
                  navigate(
                    `/claims/batches/entry?employerId=${employerId}&providerId=${providerId}&month=${month}&year=${year}&claimId=${claim.id}`
                  )
                }
              >
    ...
    <>
      <Box sx={{ display: 'flex', flexDirection: 'column', px: { xs: 2, sm: 3 }, pb: 2 }}>
        <ModernPageHeader
          ...
          icon={ReceiptIcon}
          breadcrumbs={[{ label: 'الرئيسية', path: '/' }, { label: 'نظام الدفعات', path: '/claims/batches' }, { label: batchCode }]}
          actions={
            <Stack direction="row" spacing={1.5} alignItems="center">
              <Button
                ...
                startIcon={<AddIcon />}
                onClick={() =>
                  navigate(`/claims/batches/entry?employerId=${employerId}&providerId=${providerId}&month=${month}&year=${year}`)
                }
                sx={{
              ...
              >
                ...
                >
                  ...
                  <Typography variant="body2">
                    {selectedClaimIds.length > 0 ? `طباعة (${selectedClaimIds.length})` : 'طباعة الكل'}
                  </Typography>
                </MenuItem>
        ...
        <Box sx={{ mt: -1 }}>
          <Stack spacing={1.5}>
            ...
            {showDeleted ? (
              ...
              <MainCard sx={{ p: 0 }}>
                <Box
                  sx={{ p: '12px 16px', borderBottom: '1px solid', borderColor: 'divider', display: 'flex', alignItems: 'center', gap: 1 }}
                >
                        ...
                        <tbody>
                          {deletedItems.map((c, i) => (
                            <tr key={c.id} style={{ borderBottom: '1px solid #e0e0e0', background: i % 2 === 0 ? '#fff' : '#fafafa' }}>
                              ...
                              <td style={{ padding: '8px 14px', textAlign: 'center', color: '#888', fontSize: '0.75rem' }}>
                                {c.deletedBy || '—'}
                              </td>
                              <td
                                style={{ padding: '8px 14px', textAlign: 'center', color: '#e74c3c', fontSize: '0.75rem', fontWeight: 500 }}
                              >
                                {c.voidReason || '—'}
                              </td>
              ...
              <UnifiedMedicalTable
                ...
                onSort={(col, dir) => {
                  tableState.setSorting([{ id: col, desc: dir === 'desc' }]);
                  tableState.setPage(0);
                }}
      ...
      <Dialog open={suspendDialogOpen} onClose={() => setSuspendDialogOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogActions sx={{ px: '1.5rem', pb: '1.0rem', gap: 1 }}>
          <Button variant="outlined" onClick={() => setSuspendDialogOpen(false)}>
            إلغاء
          </Button>
      ...
      <Dialog open={deleteDialogOpen} onClose={() => setDeleteDialogOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent sx={{ pt: '1.25rem' }}>
          <Typography variant="body2" mb={1}>
            هل أنت متأكد من إلغاء المطالبة الخاصة بـ <strong>{deletingClaim?.memberName}</strong> بمبلغ{' '}
            <strong>{(deletingClaim?.requestedAmount || 0).toFixed(2)}</strong>؟
          </Typography>
          <Typography variant="body2" color="text.secondary" mb={2}>
            • سيتم إخفاء المطالبة من القوائم النشطة
            <br />
            • ستعود الأموال المحجوزة إلى السقف السنوي تلقائياً
            <br />• يمكن استعادتها لاحقاً من سجل المحذوفات
          </Typography>
        </DialogContent>
        <DialogActions sx={{ px: '1.5rem', pb: '1.0rem', gap: 1 }}>
      ...
      <Dialog open={restoreDialogOpen} onClose={() => setRestoreDialogOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent sx={{ pt: '1.25rem' }}>
          <Typography variant="body2" color="text.secondary">
            هل أنت متأكد من استعادة مطالبة <strong>{restoringClaim?.memberName}</strong> بمبلغ{' '}
            <strong>{(restoringClaim?.requestedAmount || 0).toFixed(2)}</strong>؟
          </Typography>
        ...
        <DialogActions sx={{ px: '1.5rem', pb: '1.0rem', gap: 1 }}>
          <Button variant="outlined" onClick={() => setRestoreDialogOpen(false)}>
            إلغاء
          </Button>
          <Button
            variant="contained"
            ...
            onClick={() => {
              restoreMutation.mutate(restoringClaim?.id);
      ...
      <Dialog open={hardDeleteDialogOpen} onClose={() => setHardDeleteDialogOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogActions sx={{ px: '1.5rem', pb: '1.0rem', gap: 1 }}>
          <Button variant="outlined" onClick={() => setHardDeleteDialogOpen(false)}>
            إلغاء
          </Button>
          <Button
            variant="contained"
