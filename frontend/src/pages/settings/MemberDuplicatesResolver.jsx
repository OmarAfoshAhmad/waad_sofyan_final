  ...
  return (
    <Box sx={{ p: 2 }}>
      ...
      <Typography variant="body1" color="text.secondary" mb={3}>
        هذه الأداة تقوم بالبحث التلقائي عن المشتركين المكررين في النظام وتسمح لك بدمج الحركات (المطالبات والزيارات) الخاصة بهم في حساب واحد
        ثم حذف البقية.
      </Typography>
        ...
        <Box>
          ...
          >
            <Box sx={{ display: 'flex', gap: 2 }}>
              ...
              <Chip
                label={`إجمالي السجلات المكررة: ${duplicates.reduce((acc, curr) => acc + (curr.members?.length || 0), 0)}`}
                color="secondary"
                variant="outlined"
              />
            </Box>
          ...
          <Grid container spacing={3}>
            {duplicates
              .filter(
                (group) =>
                  group.normalizedName.includes(searchTerm) ||
                  (group.members &&
                    group.members.some(
                      (m) => m.cardNumber.includes(searchTerm) || (m.nationalNumber && m.nationalNumber.includes(searchTerm))
                    ))
              )
              .map((group, index) => (
                  ...
                  <Card sx={{ borderLeft: '4px solid', borderLeftColor: 'warning.main' }}>
                    <CardContent>
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                        ...
                        <Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
                          <Button
                            ...
                            onClick={() => setFamilyDialog({ open: true, group })}
                          >
                            عرض العائلة للتأكد
                          </Button>
                      ...
                      <FormControl component="fieldset" fullWidth>
                        <RadioGroup value={selectedPrimaryMap[index] || ''} onChange={(e) => handlePrimaryChange(index, e.target.value)}>
                          {group.members.map((member) => (
                            ...
                            >
                              <FormControlLabel value={member.id} control={<Radio />} label="" sx={{ mr: 1 }} />
                              <Box sx={{ flexGrow: 1 }}>
                                ...
                                <Box sx={{ display: 'flex', gap: 2, mt: 0.5 }}>
                                  ...
                                  <Typography variant="caption">
                                    تاريخ الإضافة: {new Date(member.createdAt).toLocaleDateString('ar-LY')}
                                  </Typography>
                                </Box>
                              </Box>
                              ...
                              <Box sx={{ display: 'flex', gap: 1 }}>
                                {member.visitCount > 0 && (
                                  <Chip size="small" label={`${member.visitCount} زيارات`} color="info" variant="outlined" />
                                )}
                                {member.claimCount > 0 && (
                                  <Chip size="small" label={`${member.claimCount} مطالبات`} color="error" variant="outlined" />
                                )}
                                {member.dependentCount > 0 && (
                                  <Chip size="small" label={`${member.dependentCount} تابعين`} color="success" variant="outlined" />
                                )}
                    ...
                    <Box key={m.id} sx={{ mb: 2, p: 1.5, bgcolor: 'background.default', borderRadius: 1, border: '1px solid #eee' }}>
                      ...
                      {m.dependentNames && m.dependentNames.length > 0 ? (
                        ...
                        <Typography variant="caption" color="text.secondary">
                          لا يوجد تابعين مرتبطين بهذا السجل
                        </Typography>
                      )}
                    </Box>
                  ))}
                ...
                <>
                  ...
                  <Box sx={{ p: 1.5, bgcolor: 'background.default', borderRadius: 1, border: '1px solid #eee' }}>
                    <Typography variant="body2" mb={1}>
                      الاسم: <strong>{familyDialog.group.parentName || 'غير متوفر'}</strong>
                    </Typography>
                    <Typography variant="body2">
                      رقم البطاقة: <strong>{familyDialog.group.parentCardNumber || 'غير متوفر'}</strong>
                    </Typography>
                  </Box>
