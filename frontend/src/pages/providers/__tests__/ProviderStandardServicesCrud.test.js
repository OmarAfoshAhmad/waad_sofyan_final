...
describe('provider standard services catalog CRUD', () => {
  ...
  it('requires code, name, category, and default context before allowing submit', () => {
    expect(dialogSource).toContain(
      'const isValid = form.code.trim() && form.nameAr.trim() && form.categoryId && form.defaultClaimContextCode;'
    );
    expect(dialogSource).toContain('label="سياق الاستخدام الافتراضي"');
    expect(dialogSource).toContain('defaultClaimContextCode: form.defaultClaimContextCode');
  });
