...
public class UnifiedMemberService {
    ...
    public MemberViewDto createPrincipalMember(MemberCreateDto dto) {
        ...
        String cardNumber = normalizeForComparison(dto.getCardNumber());
        if (cardNumber == null) {
            ...
            if (memberRepository.existsByCardNumber(cardNumber)) {
                throw new BusinessRuleException(
                        "رقم البطاقة مستخدم لمستفيد آخر: " + cardNumber);
            }
            if (memberRepository.existsByBarcode(cardNumber)) {
                throw new BusinessRuleException(
                        "رقم البطاقة مستخدم كباركود لمستفيد آخر: " + cardNumber);
            }
        }
    ...
    protected Member createDependentInternal(Member principal, DependentMemberDto dto) {
        ...
        // 1. Generate card number with relationship suffix unless a manual correction is supplied.
        String cardNumber = normalizeForComparison(dto.getCardNumber());
        if (cardNumber == null) {
            cardNumber = cardNumberGenerator.generateForDependent(principal, dto.getRelationship());
            log.debug("✅ Generated card number for dependent: {}", cardNumber);
        } else if (memberRepository.existsByCardNumber(cardNumber)) {
            throw new BusinessRuleException("رقم البطاقة مستخدم لمستفيد آخر: " + cardNumber);
        } else if (memberRepository.existsByBarcode(cardNumber)) {
            throw new BusinessRuleException("رقم البطاقة مستخدم كباركود لمستفيد آخر: " + cardNumber);
        }
    ...
    /**
     * Card numbers are generated by default at creation time, but operations
     * occasionally need to correct legacy/manual data. Keep barcode identical
     * to cardNumber because Member entity enforces that invariant on save.
     *
     * Blank input means "leave current generated number untouched", not clear.
     */
    private void applyCardNumberCorrection(Member member, MemberUpdateDto dto) {
        String submittedCard = normalizeForComparison(dto.getCardNumber());
        if (submittedCard == null) {
            return;
        }

        String currentCard = normalizeForComparison(member.getCardNumber());
        if (submittedCard.equals(currentCard)) {
            return;
        }

        if (memberRepository.existsByCardNumberAndIdNot(submittedCard, member.getId())) {
            throw new BusinessRuleException("رقم البطاقة مستخدم لمستفيد آخر: " + submittedCard);
        }

        String currentBarcode = normalizeForComparison(member.getBarcode());
        if (!submittedCard.equals(currentBarcode) && memberRepository.existsByBarcode(submittedCard)) {
            throw new BusinessRuleException("رقم البطاقة مستخدم كباركود لمستفيد آخر: " + submittedCard);
        }

        member.setCardNumber(submittedCard);
        member.setBarcode(submittedCard);
    }
    ...
    public MemberViewDto updateMember(Long id, MemberUpdateDto dto) {
        ...
        applyCardNumberCorrection(member, dto);
