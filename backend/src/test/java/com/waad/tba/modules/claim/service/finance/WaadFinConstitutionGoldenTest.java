 ...
 *
 * History: first written against the OLD ClaimLineFinancialEngine and was
 * RED, proving the old engine's maximumCompanyShare capped insurerFinalPayment
 * AFTER the patient/discount split instead of capping settlementBase BEFORE
 * it. It then exercised WaadFinancialEngine v1 and went GREEN. This version
 * targets WaadFinancialEngine after the finance-02.1 contract hardening
 * (LimitMode.LIMITED, insideLimit, bindingAvailableLimit/bindingRemainingLimit,
 * no approvedQuantity) -- values are unchanged, only field names are.
 */
class WaadFinConstitutionGoldenTest {
    ...
    void goldenScenario() {
        Result r = engine.evaluate(new Input(
                ...
                LimitMode.LIMITED,
                new BigDecimal("600.00"),  // bindingAvailableLimit
                80,                        // coveragePercent
        ...
        assertThat(r.insideLimit()).isEqualByComparingTo("600.00");
